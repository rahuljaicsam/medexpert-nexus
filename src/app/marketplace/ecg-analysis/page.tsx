...
const dataset = {
  ...
  rating: 4.8,
  sampleSize: 12000,
  labeledSamples: 12000,
  validationSplit: "70/15/15",
  lastUpdated: "2024-03-08",
  documentation: {
    overview: `This dataset contains high-quality ECG recordings with expert annotations:

- 12-lead ECG recordings
- Beat-by-beat annotations
- Rhythm classifications
- Detailed diagnostic interpretations
- Demographic information

The dataset covers a wide range of cardiac conditions and normal variants.`,

    dataFormat: `Data is provided in multiple formats:
- DICOM: Standard medical format with metadata
- CSV: Time series data with annotations
- JSON: Detailed annotations and metadata

Each recording includes:
- Raw signal data at 500Hz sampling rate
- Beat annotations with timestamps
- Diagnostic codes and interpretations`,

    quality: `Quality control measures:
- Expert validation by cardiologists
...
class ECGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        signal, fields = wfdb.rdsamp(f"{self.data_dir}/{idx}")
        annotations = wfdb.rdann(f"{self.data_dir}/{idx}", 'atr')
...
def create_dataset(data_path, batch_size=32):
    ...
    dataset = dataset.map(load_ecg)
    dataset = dataset.batch(batch_size)
    return dataset`
  },
  stats: [
    { month: 'Jan', downloads: 35 },
    { month: 'Feb', downloads: 55 },
    { month: 'Mar', downloads: 75 },
    { month: 'Apr', downloads: 85 },
    { month: 'May', downloads: 105 },
    { month: 'Jun', downloads: 145 },
  ],
  sampleImages: [
